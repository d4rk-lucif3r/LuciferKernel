...
git clone --depth=1  https://github.com/d4rk-lucif3r/LuciferKernel.git -b nonOC-test
cd LuciferKernel
...
function zipping() {
    ...
    zip -r9 LuciferKernel_V4_NonOC_test.zip *
    cd $SEND_DIR   || exit 1
    echo "Changing Dir to Send FIle"
    ./telegram -t $TELEGRAM_TOKEN -c $TELEGRAM_CHAT -f $REPACK_DIR/LuciferKernel_V4_NonOC_test.zip "Zip Sent through CircleCi"
