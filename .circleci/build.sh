...
function zipping() {
    ...
    zip -r9 LuciferKernel_V3_test-NonOC.zip *
    cd $SEND_DIR   || exit 1
    echo "Changing Dir to Send FIle"
    ./telegram -t $TELEGRAM_TOKEN -c $TELEGRAM_CHAT -f $REPACK_DIR/LuciferKernel_V3_test-NonOC.zip "File Sent through CircleCi"
